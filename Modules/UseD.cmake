...
function(set_d_versions)
    cmake_parse_arguments(ARG "" "TARGET" "" ${ARGN})
    foreach(vers IN LISTS ARG_UNPARSED_ARGUMENTS)
        if(ARG_TARGET)
            ...
            #set(D_VERSIONS "${D_VERSIONS} ${CMAKE_D_VERSION_FLAG}${vers}")
        endif()
    endforeach()
    set(CMAKE_D_FLAGS "${CMAKE_D_FLAGS}" PARENT_SCOPE)
    #set(D_VERSIONS "${D_VERSIONS}" PARENT_SCOPE)
endfunction()

function(d_target_link_libraries _target)
    get_target_property(sources "${_target}" SOURCES)
    list(GET sources 0 source)
    if(CMAKE_D_LINK_FLAG AND source MATCHES ".*\\.d")
        foreach(lib IN LISTS ARGN)
            if(EXISTS "${lib}" AND NOT IS_DIRECTORY "${lib}")
                list(APPEND linkflags "${CMAKE_D_LINK_FILE_FLAG}${lib}")
            elseif(TARGET "${lib}")
                get_target_property(out "${lib}" LOCATION)
                file(TO_NATIVE_PATH "${out}" out)
                list(APPEND linkflags "${CMAKE_D_LINK_FILE_FLAG}${out}")
                add_dependencies(${_target} "${lib}")
            elseif(lib MATCHES "[-/].*")
                list(APPEND linkflags "${lib}")
            else()
                list(APPEND linkflags "${CMAKE_D_LINK_FLAG}${lib}")
            endif()
        endforeach()
    else()
        list(APPEND linkflags ${ARGN})
    endif()
    target_link_libraries(${_target} ${linkflags})
endfunction()
...
function(autod _target)
    ...
    set_d_versions(TARGET ${_target} ${ARG_VERSIONS})
