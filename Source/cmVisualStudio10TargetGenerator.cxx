...
void cmVisualStudio10TargetGenerator::WriteDProj()
{
  ...
  for(std::vector<std::string>::iterator i = configs->begin();
    ...
    if(!impList.empty())
      {
      //cmSystemTools::ExpandListArgument(imps, impList);
...
#ifdef __CMAKE_D_VS_LIBFILES
    this->WriteString("<libfiles>", 2);
    cmTarget::LinkLibraryVectorType libs = this->Target->GetLinkLibraries();
    for(cmTarget::LinkLibraryVectorType::iterator i = libs.begin();
        i != libs.end(); ++i)
      {
      (*this->BuildFileStream) << " " << (i->first.c_str() + 4);
      }
